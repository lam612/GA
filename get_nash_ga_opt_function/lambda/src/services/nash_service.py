...
class NashService:
    ...
    def process(self, vmi_id, context):
        ...
        while True:
            ...
            # Check nash
            is_nash_eq = self.check_nash_eq(step)

            if (is_nash_eq or step > CommonConfig.MAX_NASH_GA_LOOP) and step > CommonConfig.MIN_START_CHECK_NASH_STEP:
    ...
    def gen_random_solution(self):
        while True:
            ...
            cp_list = [random.randrange(
                int(material_cost) * p_e[idx], int(price * 0.85)) for idx, price in enumerate(CommonConfig.PRODUCT_PRICE)]

            a_list = [random.randrange(1, CommonConfig.MAX_a)
                      for _ in range(self.mf_dao.NUM_OF_RETAILERS)]
            if self.pre_demand.get_total_predict(A_list, a_list, cp_list) < self.mf_dao.P:
                strategy = [A_list, cp_list, a_list]
                solution = {
                    "fitness": 0,
                    "solution": strategy
                }
    ...
    def get_nash_ga_response(self, vmi_id, context, is_nash_eq, step):
        ...
        if is_nash_eq:
            ...
            self.vmi_val_service.update_item(
                vmi_id, JobStatus.SUCCEED, json.dumps(cur_strategy), json.dumps(self.nash_strategy))
            ...
            self.vmi_val_service.update_item(
                vmi_id, JobStatus.CLOSED, json.dumps(cur_strategy), json.dumps(self.nash_strategy))

        print("[A] {}\n[a] {}\n[S] {}\n[F] {}\n[D] {}\n[P] {}\n".format(
            A, a, cp, fitness, demand, profit))
    ...
    def calc_next_solution(self):
        ...
        for player_id in self.nash_dao.nash.keys():
            if player_id == self.nash_dao.mf_id:
                dump_A_list = [0] * self.mf_dao.NUM_OF_RETAILERS
                dump_cp_list = [0] * self.mf_dao.NUM_OF_PRODUCT
                # Next solution : A, cp_list, a_list*
                next_solution[player_id] = [
                    dump_A_list, dump_cp_list, pre_r_ads]
            else:
    ...
    def compare_fitness(self, nash_str_fitness, target_str_fitness):
        ...
        for idx, _ in enumerate(nash_str_fitness):
            ...
            if gama > max_error:
                ...
                print("[➣] Gama : {:^2.2f}% | Error : {:^2.2f}% | {} | {} - {}".format(gama * 100, max_error * 100,
                      error, nash_str_fitness[idx], target_str_fitness[idx]))
                return idx + 10
        print("\n[✔︎] {}\n[✔] {}".format(
            nash_str_fitness, target_str_fitness))
        print("[➣] Gama : {:^2.2f}% | Error : {:^2.2f}% | {} | {} - {}".format(gama * 100, max_error * 100,
                                                                               error, nash_str_fitness[idx], target_str_fitness[idx]))
        return 1

    def check_nash_eq(self, step):
    ...
    def get_cur_nash_eq(self):
        pre_mf_ads, pre_mf_cp, pre_r_ads = [], [], []
        r_idx = -1
        for player_id in self.nash_dao.nash.keys():
        ...
        # print("- Profit : {} | TR : {} | TC : {} | D : {}".format(self.mf_service.get_m_profit(),
        #       self.mf_service.get_TR_M(), self.mf_service.get_TC_M(), self.mf_service.get_retailers_demand()))
