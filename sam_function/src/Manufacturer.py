...
class Manufacturer:
    ...
    def __init__(self, A=int, n=int, x=int):
        self.cm = config.cm
        self.cr = config.cr
        self.H_p = config.H_p
        self.H_r = config.H_r
    ...
    def get_total_demand(self):
        return sum([retailer.get_predict_demand(self.A) for retailer in self.retailers])

    def add_retailer(self, retailer):
        self.retailers.append(retailer)

    def set_retailers_ads(self, a_list):
        for idx, retailer in enumerate(a_list):
            retailer.set_ads(a_list[idx])

    def set_retailers_sell_price(self, sell_price_list):
        for idx, retailer in enumerate(a_list):
            retailer.p = sell_price_list[idx]
    ...
    def get_rt_profit(self, rt_id):
        return self.retailers[rt_id].get_profit(self.A)
    ...
    def get_TR_M(self):
        ...
        for i in range(len(self.retailers)):
            demand_i = self.retailers[i].get_predict_demand(self.A)
